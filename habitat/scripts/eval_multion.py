...
def main():
    ...
    if args.agent == "frontier_sgnav":
        ...
        if args.ground_truth_scene_graph:
            flag_gt_sg = "GTSG_"
        args.exp_name = (
            f"multion_{args.agent}_{args.goal_policy}_priorw_{args.util_prior_combine_weight}_"
    ...
    # for step in range(args.total_steps): # should not trigger this stop in multion
    # while(True)
    for episode_idx in range(num_episodes):
        ...
        if done:
            ...
            # collect metrics for one episode 
        ...
        if episode_idx % args.log_interval == 0:
            ...
            log = " ".join(
                [
                    ...
                    "num timesteps {},".format(episode_idx * num_scenes),
                    "FPS {},".format(int(episode_idx * num_scenes / (end - start))),
                ]
            )
    ...
    if args.eval:
        ...
        with open("{}/{}result.json".format(dump_dir, args.split), "w") as f:
