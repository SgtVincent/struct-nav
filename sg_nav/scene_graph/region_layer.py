...
class RegionLayer:
    ...
    def add_region(self, bbox, id=None, class_name=None, label=None):
        ...
        if id == None or id in self.region_ids:
            ...
            print(f"Warning: region id {id} already exists in region layer. Assign id {new_id} instead")
            id = new_id
    ...
    def segment_region_on_grid_map_xz(self, region_id, region_bbox):
        ...
        region_2d_bbox = project_points_to_grid_xz(self.bounds, region_bbox, self.grid_size)
        
        region_mask = np.zeros_like(self.free_space_grid).astype(bool)
        # NOTE: (row_idx, col_idx) corresponds to (y, x) in 2d grid map 
        region_mask[int(np.ceil(region_2d_bbox[0][1])):int(np.floor(region_2d_bbox[1][1])),
            int(np.ceil(region_2d_bbox[0][0])):int(np.floor(region_2d_bbox[1][0]))] = True
