...
class ObjectNode:
    ...
    def __init__(self, id, center, rotation, size, 
        class_name=None,
        label=None,
        parent_region=None,
        vertices=None, 
    ...
    def add_point_clouds(self, vertices, colors, normals):
        ...
        # FIXME: realtime vertex normal estimation is EXTREMELY time-costly
        # self.o3d_pcl = o3d.geometry.PointCloud()
        # self.o3d_pcl.points = o3d.utility.Vector3dVector(vertices)
        # self.o3d_pcl.colors = o3d.utility.Vector3dVector(colors)
        # if normals:
        #     self.o3d_pcl.normals = o3d.utility.Vector3dVector(normals)
        # else: # use estimated surface normals 
        #     self.o3d_pcl.estimate_normals(search_param=o3d.geometry.KDTreeSearchParamHybrid(radius=0.1, max_nn=30))
        self.vertices = vertices
        self.colors = colors
        self.normals = normals
...
class ObjectLayer:
    ...
    def init_map(self,         
        ...
        self.bounds = bounds # the real map area corresponding to free space grid
        self.grid_size = grid_size
        self.segment_grid = np.zeros_like(free_space_grid, dtype=int) - 1
        self.free_space_grid = np.array(free_space_grid).astype(bool)
        self.project_mode = project_mode # 'xz' means project xz axes in 3D to xy axes in map 
    ...
    def add_object(self, center, rotation, size, id=None, class_name=None, label=None,
     pcls=None, normals=None, bbox=None):
        # add object node 
        if id == None or id in self.obj_ids:
            new_id = 0
            if len(self.obj_ids) > 0:
                new_id = max(self.obj_ids)+1
            print(f"Warning: object id {id} already exists in object layer. Assign id {new_id} instead")
            id = new_id
        
        self.obj_ids.append(id)
        obj_node = ObjectNode(id, center, rotation, size, class_name=class_name, label=label)
    ...
    def segment_object_on_grid_map_xz(self, obj_id, obj_bbox):
        assert self.flag_grid_map, "called 'segment_region_on_grid_map_xz()' before grid map being initialized"
        # get region bbox on 2d grid map 
        # object_2d_bbox: np.array: [[x_min, y_min],[x_max, y_max]]
        obj_2d_bbox = project_points_to_grid_xz(self.bounds, obj_bbox, self.grid_size)
        
        obj_mask = np.zeros_like(self.free_space_grid).astype(bool)
        # NOTE: (row_idx, col_idx) corresponds to (y, x) in 2d grid map 
        obj_mask[int(np.ceil(obj_2d_bbox[0][1])):int(np.floor(obj_2d_bbox[1][1])),
            int(np.ceil(obj_2d_bbox[0][0])):int(np.floor(obj_2d_bbox[1][0]))] = True
        # color the region on global grid map 
        if (self.segment_grid[obj_mask] == -1).all():
            self.segment_grid[obj_mask] = obj_id
        else:
            print(f"Warning: object {obj_id} overlap with object {self.segment_grid[obj_mask]}")
            self.segment_grid[obj_mask] = obj_id
