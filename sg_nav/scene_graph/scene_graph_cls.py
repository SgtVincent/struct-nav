...
class SceneGraphHabitat(SceneGraphBase):
    ...
    def __init__(self, config, scene_name=None) -> None:
        ...
        self.meters_per_grid = self.config.meters_per_grid # free space grid parameter
    ...
    def load_gt_scene_graph(self, ply_file, pclseg_file, pcl_normals_file, sim:Simulator):
        
        # 0. load groun truth 3D point clouds and class-agnostic instance segmentation 
        o3d_pcl = o3d.io.read_point_cloud(ply_file) # open scene mesh file for segmentation and feature extraction 
        points = np.concatenate([np.asarray(o3d_pcl.points), np.asarray(o3d_pcl.colors)], axis=1)
        ...
        self.free_space_grid = sim.pathfinder.get_topdown_view(self.meters_per_grid, self.height) # binary matrix
        self.region_layer.init_map(self.scene_bounds, self.meters_per_grid, self.free_space_grid)

        self.dumy_space_grid = sim.pathfinder.get_topdown_view(self.meters_per_grid / self.object_grid_scale,
         self.height) # binary matrix
        self.object_layer.init_map(self.scene_bounds, self.meters_per_grid / self.object_grid_scale,
        ...
        for region in tqdm(semantic_scene.regions):
            ...
            region_bbox = np.stack([region.aabb.center - region.aabb.sizes/2,
                region.aabb.center + region.aabb.sizes/2], axis=0)
            region_node = self.region_layer.add_region(region_bbox, id=sg_region_id, class_name=region.category.name(), label=region.category.index())
            ...
            for obj in region.objects:
                ...
                node_size = self.meters_per_grid / self.object_grid_scale # treat object as a point
                node_bbox = np.stack([center - node_size/2,
                    center + node_size/2], axis=0)
                object_node = self.object_layer.add_object(center, rot_quat, size, id=object_id, 
                    class_name=obj.category.name(), label=obj.category.index(),
                    pcls=object_points, normals=object_normals, bbox=node_bbox)
    ...
    def get_full_point_clouds(self):
        ...
        for obj_id in self.object_layer.obj_ids:
            ...
            obj_points = np.concatenate([obj_node.vertices, obj_node.colors, obj_node.normals], axis=1)
            points.append(obj_points)
                
            points_object_ids.append(np.repeat(obj_id, obj_points.shape[0]).astype(int)) 
