    ...
    def data_process(self, submaps):
        ...
        for i, submap in enumerate(submaps):
            ...
            choice = np.random.choice(
                len(pcls), self.num_points, replace=len(pcls) < self.num_points
            )
            pcls = pcls[choice, :]
        ...
        return (
            sampled_points.float().to(self.device),
            edges.to(self.device),
            descriptors.float().to(self.device),
        )
    ...
    def predict(self, object_nodes, return_input=False, return_nparr=False):
        ...
        if self.model_type == "SGFN":
            ...
            if len(edges) > 0:
                ...
                pred_obj_confidence, pred_obj_label = torch.max(
                    pred_obj_prob, dim=1
                )

                pred_rel_prob = torch.exp(pred_rel_cls)
                pred_rel_confidence, pred_rel_label = torch.max(
                    pred_rel_prob, dim=1
                )
