...
from scene_graph.utils import display_image_arr, inNd, grid_xz_to_points
...
class PyGDatasetWrapper():
    def __init__(self, dataset, node_features, edges, map_ids2idx, add_loops=True):
    ...
    def process_data(self, data):
        sample_obj_ids, objnav_labels, target_obj_inout_labels, fs_points, fs2obj_geo_dist = data
        sample_idx = sample_obj_ids.detach().clone().apply_(self.map_ids2idx.get)
        inputs = self.node_features[sample_idx]
        if self.add_loops:
            edges, _ = remove_self_loops(self.edges)
            edges, _ = add_self_loops(self.edges)
        sg_edge_index = self.get_sg_edge_idx(self.node_features, sample_idx, edges)
        return Data(x=inputs, edge_index=sg_edge_index, y=target_obj_inout_labels, n_y=objnav_labels)

    @staticmethod
    def get_sg_edge_idx(node_features, sample_idx, edges):
            map_gidx2sgidx = {gidx: sgidx for sgidx, gidx in enumerate(sample_idx.numpy())}
            node_mask = torch.zeros(node_features.size(0), dtype=torch.bool, device=node_features.device)
            node_mask[sample_idx] = 1
            edge_mask = node_mask[edges[0]] & node_mask[edges[1]]
            sample_edge_index = edges[:, edge_mask]
            sg_edge_index = \
                sample_edge_index.detach().clone().cpu().apply_(map_gidx2sgidx.get).to(node_features.device)
            return sg_edge_index

class DataSampler(Dataset):
    """Sample data from habitat scene graphs."""
    def __init__(self, config, scene_graph, save_dir, obj_nav_class_list,
                 blk_size_min=8, sample_sizes_max=10000, source_sample_size=64) -> None:
        ...
        for c in self.obj_nav_class_list:
            ...
            if geo_map.size != 0:
                ...
                class_geo_map = np.full_like(
                    self.map_meta_dict['free_space_grid'],
                    np.inf, dtype=float)
                class_geo_maps.append(class_geo_map)
            if display:
    ...
    def generate_sample(self):
        ...
        for data in self.block_sampling(self.blk_size_min, self.sample_sizes_max):
            ...
            fs2obj_geo_dist = np.clip(self.get_geo_dist(
                fs_idx_arr), a_min=0., a_max=100.)
            sample = [sample_obj_ids, objnav_labels,
                      target_obj_inout_labels, fs_points, fs2obj_geo_dist]
    ...
    def block_sampling(self, blk_size_min=8, sample_sizes=1000):
        ...
        blk_tops = np.random.randint(0, map_h-blk_size_hs+1, size=sample_sizes)
        ...
        for i in range(sample_sizes):
            ...
            free_space_mask[blk_top:blk_top+blk_size_h,
                            blk_left:blk_left+blk_size_w] = False
            ...
            object_node_mask[
                blk_top*self.object_grid_scale:(blk_top+blk_size_h)*self.object_grid_scale,
                blk_left*self.object_grid_scale:(blk_left+blk_size_w)*self.object_grid_scale] = False
            ...
            if display:
                display_image_arr((2, 2), [free_space_grid, free_space_block,
                                           object_node_grid, object_node_block])
            if verbose:
                print(obj_ids)
    ...
    def get_inout_labels(self, obj_labels):
        ...
        return np.asarray([1 if c in obj_labels else 0 for c in self.obj_nav_class_list])

    def get_objnav_labels(self, obj_labels):
        "Convert object labels to object navigation labels."
        return np.asarray([self.obj_nav_class_list.index(c) if c in self.obj_nav_class_list else len(self.obj_nav_class_list) for c in obj_labels])

    def get_free_space_points(self, idx_arr):
        "Get free space points."
        free_space_points = grid_xz_to_points(self.scene_graph.scene_bounds, idx_arr[:, [1, 0]],
                                              self.scene_graph.height, self.scene_graph.meters_per_grid)
