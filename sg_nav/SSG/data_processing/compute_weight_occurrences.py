...
def Parser():
    ...
    parser.add_argument(
        "--root", type=str, default="../data/example_data", help="rio path"
    )
    parser.add_argument(
        "--type",
        type=str,
    ...
    parser.add_argument(
        "--txt",
        type=str,
        default='"../data/train_scans.txt"',
        help="path to the txt file contain scan ids",
        required=False,
    )
    return parser
...
def compute_weights(labels, classes, count, verbose=False):
    ...
    for c in range(len(classes)):
        if classes[c] / count > 0:
            ...
            if verbose:
                print(
                    "{0:>20s} {1:>1.3f} {2:>6d}".format(
                        labels[c], weight, int(classes[c])
                    )
                )
...
def compute(
    ...
    if verbose:
        print("num multi predicates:", cnn)
...
def read_relationships_json(args):
    ...
    relationNames = util.read_relationships(
        os.path.join(args.root, "relationships.txt")
    )
    pth_relationships_json = os.path.join(
        os.path.join(args.root, "relationships_" + str(args.type) + ".json")
    )

    with open(pth_relationships_json, "r") as read_file:
        data = json.load(read_file)

    wobjs, wrels, o_obj_cls, o_rel_cls = compute(
        classNames, relationNames, data, verbose=True
    )
