...
def Parser(add_help=True):
    ...
    parser.add_argument(
        "--scans", type=str, default="/media/junting/Elements/3RScan/3RScan/"
    )
    parser.add_argument(
        "--type",
        type=str,
    ...
    # split parameters
    parser.add_argument(
        "--split", type=int, default=0, help="Split scene into groups."
    )
    parser.add_argument(
        "--radius_seed",
        type=float,
        default=1,
        help="The minimum distance between two seeds.",
    )
    parser.add_argument(
        "--min_segs",
        type=int,
        default=5,
        help="Minimum segments for each segGroup",
    )
    parser.add_argument(
        "--split_method",
        type=str,
...
def generate_groups(
    ...
    if args.verbose:
        print("num of selected point seeds:", len(selected_indices))
    ...
    if sample_method == SAMPLE_METHODS.BBOX:
            ...
            if debug:
                ...
                for segment_id in segment_ids:
                    ...
                    for idx in segment_indices:
                        cloud.visual.vertex_colors[idx][:3] = seg_colors[
                            segment_id
                        ]
        ...
        for idx in seg_ids:
            ...
            bboxes[idx] = [
                segs[idx].min(0) - radknn,
                segs[idx].max(0) + radknn,
            ]
        ...
        for idx in selected_indices:
            ...
            if len(neighbors) < min_seg_per_group:
                continue
...
def process(
    ...
    if split_scene:
        seg_groups = generate_groups(
            cloud_gt,
            args.radius_seed,
            args.radius_receptive,
            args.min_segs,
    ...
    instance2labelName = util.load_semseg(
        pth_semseg_file, label_name_mapping, args.mapping
    )
    ...
    if seg_groups is not None:
        for split_id in range(len(seg_groups)):
            ...
            if (
                ...
                or len(relationships["relationships"]) == 0
            ):
                continue
...
def gen_relationship(
    ...
    if gt_relationships is not None:
        relationships_names = util.read_relationships(
            os.path.join(define.FILE_PATH, args.relation + ".txt")
        )
...
if __name__ == "__main__":
    ...
    counter = 0
    with open(
        os.path.join(define.FILE_PATH, args.relation + ".json"), "r"
    ) as read_file:
        data = json.load(read_file)
        for s in tqdm(data["scans"]):
            ...
            if len(relationships) == 0:
                print(
                    "skip", scan_id, "due to not enough objs and relationships"
                )
                continue
            else:
