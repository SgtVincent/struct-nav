...
parser.add_argument(
    ...
    type=str,
    default="",
    help="target scan_id. leave empty to process all scans",
)
...
def main():
    ...
    if os.path.exists(rescan_file_name):
        ...
        with open(rescan_file_name, "r") as f:
            for line in f:
                ...
                file_in = os.path.join(
                    define.DATA_PATH, scan_id, define.LABEL_FILE_NAME_RAW
                )
                file_out = os.path.join(
                    define.DATA_PATH, scan_id, define.LABEL_FILE_NAME
                )

                if os.path.exists(file_out) is True:
                    continue
                if scan_id in rescan2ref:  # if not we have a hidden test scan
                    if opt.thread > 1:
                        ...
                        result = pool.apply_async(
                            resave_ply,
                            (file_in, file_out, rescan2ref[scan_id]),
                        )
                    else:
                        print(text)
        ...
        while pool._cache:
            print(
                "\r{} {:2.2%}".format(
                    process_text[1 - len(pool._cache)],
                    1 - len(pool._cache) / len(process_text),
                ),
                flush=True,
                end="",
            )
