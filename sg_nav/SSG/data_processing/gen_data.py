...
def process(
    ...
    if args.v2:
        pth_gt = os.path.join(
            pth_3RScan, scan_id, "labels.instances.align.annotated.v2.ply"
        )
    else:
        pth_gt = os.path.join(
            pth_3RScan, scan_id, "labels.instances.align.annotated.ply"
        )
    ...
    if verbose or debug:
        print(
            "There are {} segments have found their correponding GT segments.".format(
                len(count_seg_pd_2_corresponding_seg_gts)
            )
        )
...
def gen_relationship(
    ...
    if gt_relationships is not None:
        ...
        for rel in gt_relationships:
            ...
            if (
                id_src in gt_segments_2_pd_segments
                and id_tar in gt_segments_2_pd_segments
            ):
                ...
                if debug:
                    if id_src in gt_segments_2_pd_segments:
                        print(
                            ...
                            "id_src",
                            id_src,
                            "is not in the gt_segments_2_pd_segments",
                        )
