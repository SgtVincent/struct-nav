...
from SSG.utils import define, util
...
def getLabelNames(path):
    ...
    with open(path, newline="") as csvfile:
        ...
        for row in spamreader:
            ...
            # break
    return (
        dict(sorted(Scan3R.items())),
        dict(sorted(NYU40.items())),
        dict(sorted(Eigen13.items())),
        dict(sorted(RIO27.items())),
        dict(sorted(RIO7.items())),
    )
...
def getLabelMapping(label_type: str, pth_mapping: str = ""):
    ...
    (
        ...
        toNameRIO27,
        toNameRIO7,
    ) = getLabelNameMapping(pth_mapping)
    IdxScan3R528, toNYU40, toEigen13, toRIO27, toRIO7 = getLabelIdxMapping(
        pth_mapping
    )
    label_names = None
    ...
    if label_type == "nyu40":
        ...
        label_names = {
            i + 1: SCANNET20_Label_Names[i]
            for i in range(len(SCANNET20_Label_Names))
        }
        for name, name40 in label_name_mapping.items():
            label_name_mapping[name] = (
                name40 if name40 in label_names.values() else "none"
            )
