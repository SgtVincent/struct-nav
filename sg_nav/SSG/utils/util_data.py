...
def build_edge_from_selection(node_ids, neighbor_dict, max_edges_per_node):
    ...
    for s_idx in node_ids:
        ...
        nn = set(nn).intersection(
            node_ids
        )  # only the nodes within node_ids are what we want
        if s_idx in nn:
...
def build_neighbor(
    nns: dict, instance2labelName: dict, n_times: int, n_seed=1
):
    """Select node"""
    ...
    for n in range(n_times):
        ...
        for node_idx in n_seletected_nodes[n]:
            ...
            found = found.intersection(
                selected_nodes
            )  # only choose the node within our selections
            if len(found) == 0:
                continue
...
def data_preparation(
    ...
    if sample_in_runtime:
        if not use_all:
            filtered_nodes = build_neighbor(
                nns, instance2labelName, num_nn, num_seed
            )
        else:
            selected_nodes = list(instance2labelName.keys())
            filtered_nodes = selected_nodes  # use all nodes
        edge_indices = build_edge_from_selection(
            filtered_nodes, nns, max_edges_per_node=-1
        )

        if num_max_rel > 0:
            choices = np.random.choice(
                range(len(edge_indices)), num_max_rel
            ).tolist()
    ...
    num_objects = (
        ...
        else len(selected_instances)
    )

    masks = np.array(
        list(map(lambda l: instance2mask[l], instances)), dtype=np.int32
    )
    ...
    if for_train:
        ...
        if multi_rel_outputs:
            adj_matrix_onehot = np.zeros(
                [num_objects, num_objects, len(relationships)]
            )
        else:
        ...
        if multi_rel_outputs:
            gt_rels = torch.zeros(
                len(edge_indices), len(relationships), dtype=torch.float
            )
        else:
    ...
    for e in range(len(edge_indices)):
        ...
        choice = np.random.choice(
            len(pointset), num_points_union, replace=True
        )
