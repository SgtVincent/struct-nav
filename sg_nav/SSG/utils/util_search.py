...
def find_neighbors(
    ...
    for idx in seg_ids:
        ...
        bboxes[idx] = [
            segs_pts[idx][:, :3].min(0) - receptive_field,
            segs_pts[idx][:, :3].max(0) + receptive_field,
        ]
    ...
    if search_method == SAMPLE_METHODS.BBOX:
        ...
        for seg_id in seg_ids:
            ...
            neighbors = list(
                f_nn(seg_id, trees, bboxes, segs_pts, receptive_field)
            )
