...
def get_metrics(label_id, confusion, VALID_CLASS_IDS: list = None):
    ...
    precision = (
        float("nan")
        if (tp + fp) == 0
        else (float(tp) / (tp + fp), tp, tp + fp)
    )
    recall = (
        float("nan")
        if (tp + fn) == 0
        else (float(tp) / (tp + fn), tp, tp + fn)
    )
...
def evaluate_topk_object(objs_target, objs_pred, k=-1):
    ...
    for obj in range(size_o):
        ...
        gt = objs_target[obj]
        index = (
            sorted(torch.where(sorted_conf == obj_pred[gt])[0])[0].item() + 1
        )
        top_k.append(index)
...
def evaluate_topk_predicate(
    gt_edges, rels_pred, multi_rel_outputs, threshold=0.5, k=-1
):
    """
...
def evaluate_topk(
    gt_rel, objs_pred, rels_pred, edges, multi_rel_outputs, threshold=0.5, k=40
):
    top_k = list()
    objs_pred = np.exp(objs_pred.detach().cpu())
    if multi_rel_outputs:
    ...
    for edge in range(len(edges)):
        ...
        sorted_conf_matrix, sorted_args_1d = torch.sort(
            conf_matrix_1d, descending=True
        )  # 1D
        if k < 1:
...
def evaluate_topk_recall(
    ...
    top_k_predicate,
    multi_rel_outputs,
    objs_pred: torch.tensor,
    objs_target: torch.tensor,
    rels_pred: torch.tensor,
    ...
    gt_edges = get_gt(
        objs_target, rels_target, edges, instance2mask, multi_rel_outputs
    )
    top_k += evaluate_topk(
        gt_edges, objs_pred, rels_pred, edges, multi_rel_outputs
    )  # class_labels, relationships_dict)
    top_k_obj += evaluate_topk_object(objs_target, objs_pred)
    top_k_predicate += evaluate_topk_predicate(
        gt_edges, rels_pred, multi_rel_outputs
    )
    return top_k, top_k_obj, top_k_predicate


def get_mean_metric(
    confusion: np.array, VALID_CLASS_IDS: list, CLASS_LABELS: list
):
    ious = dict()
    precisions = dict()
    recalls = dict()
...
def write_result_file(
    ...
    with open(filename, "w") as f:
        ...
        for r in range(len(VALID_CLASS_IDS)):
            f.write(
                "{0:<14s}({1:<2d})".format(CLASS_LABELS[r], VALID_CLASS_IDS[r])
            )
...
def build_edge2name_value(
    values: torch.tensor, edges: torch.tensor, idx2gtcls: dict, names: list
):
    if edges.shape[0] == 2:
        edges = edges.t()
    nn2v = dict()
...
class EvaPairWeight:
    def __init__(self, class_names: list):
        ...
        self.c_mat = np.zeros(
            [len(self.class_names), len(self.class_names)], dtype=np.float
        )
...
class EvaClassification:
    ...
    def update(
        self, pd_indices: dict, gt_indices: dict, gt_only=False, pd_only=False
    ):
        ...
        for (
            ...
            union_indices
        ):  # since it's union, there is no way to get both unknown.
            if not multi_pred:
                pd = self.unknown if idx not in pd_indices else pd_indices[idx]
                pd_idx = (
                    self.class_names.index(pd)
                    if pd in self.class_names
                    else self.unknown
                )
                gt = self.unknown if idx not in gt_indices else gt_indices[idx]
                gt_idx = (
                    self.class_names.index(gt)
                    if gt in self.class_names
                    else self.unknown
                )
                ...
                def get_indices(indices) -> list:
                    if idx not in indices:
                        ...
                        idxes = [
                            self.class_names.index(i) for i in indices[idx]
                        ]
                    return idxes

                pd_indices_set = set(get_indices(pd_indices)).difference(
                    [self.unknown]
                )
                gt_indices_set = set(get_indices(gt_indices)).difference(
                    [self.unknown]
                )
                        ...
                        for pd_idx in diff_pd:
                            if len(diff_gt) > 0:
                                for gt_idx in diff_gt:
                                    self.c_mat[gt_idx][pd_idx] += 1 / len(
                                        diff_gt
                                    )
                            else:
...
class EvalSceneGraph:
    def __init__(
        self,
        obj_class_names: list,
        rel_class_names: list,
        multi_rel_outputs: float = 0.5,
        k=100,
        multi_rel_prediction: bool = True,
    ):
        # params
    ...
    def add(
        ...
        if rel_pds is not None and rel_pds.shape[0] > 0:
            ...
            pd["edges"] = build_edge2name(
                r_pd, edge_indices, idx2seg, self.rel_class_names
            )
            gt["edges"] = build_edge2name(
                rel_gts, edge_indices, idx2seg, self.rel_class_names
            )
        ...
        if self.k > 0:
            ...
            if rel_pds is not None:
                gt_edges = get_gt(
                    ...
                    edge_indices,
                    seg2idx,
                    self.multi_rel_prediction,
                )
                self.top_k_rel += evaluate_topk_predicate(
                    gt_edges,
                    rel_pds,
                    multi_rel_outputs=self.multi_rel_prediction,
                    threshold=self.multi_rel_outputs,
                    k=self.k,
                )

                self.top_k_triplet += evaluate_topk(
                    gt_edges,
                    obj_pds,
                    rel_pds,
                    edge_indices,
    ...
    def get_mean_metrics(self):
        return (
            self.eva_o_cls.get_mean_metrics(),
            self.eva_r_cls.get_mean_metrics(),
        )
    ...
    def write(self, path, model_name):
        ...
        obj_results = write_result_file(
            self.eva_o_cls.c_mat,
            os.path.join(path, model_name + "_results_obj.txt"),
            [],
            self.eva_o_cls.class_names,
        )
        rel_results = write_result_file(
            self.eva_r_cls.c_mat,
            os.path.join(path, model_name + "_results_rel.txt"),
            [],
            self.eva_r_cls.class_names,
        )
