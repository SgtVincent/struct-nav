    ...
    if pth_label != pth_label_raw:
        ...
        if "nx" not in label_mesh_align.metadata["ply_raw"]["vertex"]["data"]:
            ...
            label_mesh_align.metadata["ply_raw"]["vertex"]["data"][
                "nx"
            ] = np.zeros([len(query_points), 1], dtype=np.float32)
            label_mesh_align.metadata["ply_raw"]["vertex"]["data"][
                "ny"
            ] = np.zeros([len(query_points), 1], dtype=np.float32)
            label_mesh_align.metadata["ply_raw"]["vertex"]["data"][
        ...
        label_mesh_align.metadata["ply_raw"]["vertex"]["data"][
            "red"
        ] = mesh.visual.vertex_colors[:, 0]
        label_mesh_align.metadata["ply_raw"]["vertex"]["data"][
            "green"
        ] = mesh.visual.vertex_colors[:, 1]
        label_mesh_align.metadata["ply_raw"]["vertex"]["data"][
            "blue"
        ] = mesh.visual.vertex_colors[:, 2]
