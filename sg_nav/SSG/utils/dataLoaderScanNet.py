...
def load_scannet(pth_ply, pth_agg, pth_seg, verbose=False, random_color=False):
    ...
    segment_ids = list(
        np.unique(np.array(segs["segIndices"]))
    ...
    plydata, instances = scannet_get_instance_ply(
        plydata, segs, aggre, random_color=random_color
    )
    ...
    for scan_id in sorted(ids):
        ...
        plydata = trimesh.load(
            os.path.join(
                ...
                scan_id,
                scan_id + define.SCANNET_PLY_SUBFIX,
            ),
            process=False,
        )
        ...
        with open(
            os.path.join(
                define.SCANNET_DATA_PATH,
                scan_id,
                scan_id + define.SCANNET_SEG_SUBFIX,
            )
        ) as f:
            segs = json.load(f)
        ...
        assert aggre["sceneId"].split("scannet.")[1] == scan_id
        assert (
            aggre["segmentsFile"].split("scannet.")[1]
            == scan_id + define.SCANNET_SEG_SUBFIX
        )
