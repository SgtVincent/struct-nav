...
from SSG.src.network_PointNet import (
    PointNetCls,
    PointNetfeat,
    PointNetRelCls,
    PointNetRelClsMulti,
...
class SGFNModel(BaseModel):
    def __init__(
        self, config: Config, name: str, num_class, num_rel, dim_descriptor=11
    ):
    ...
    def process(
        ...
        obj_pred, rel_pred, _, _, _, _, probs = self(
            obj_points,
            edges,
            descriptor,
        ...
        if self.mconfig.multi_rel_outputs:
            if self.mconfig.WEIGHT_EDGE == "BG":
                if self.mconfig.w_bg != 0:
                    weight = (
                        self.mconfig.w_bg * (1 - gt_rel_cls)
                ...
                weight[torch.where(weight == 0)] = (
                    weight[0].clone() if not ignore_none_rel else 0
                )  # * 1e-3
                weight = weight[1:]
            ...
            loss_rel = F.binary_cross_entropy(
                rel_pred, gt_rel_cls, weight=weight
            )
        else:
            if self.mconfig.WEIGHT_EDGE == "DYNAMIC":
                ...
                batch_mean = torch.sum(
                    one_hot_gt_rel, dim=(0), dtype=torch.float
                )
                weight = torch.abs(
                    1.0 / (torch.log(batch_mean + 1) + 1)
                )  # +1 to prevent 1 /log(1) = inf
            ...
            if "ignore_entirely" in self.mconfig and (
                ...
                loss_rel = torch.zeros(
                    1, device=rel_pred.device, requires_grad=False
                )
            else:
        ...
        logs = [
            ("Loss/cls_loss", loss_obj.detach().item()),
...
if __name__ == "__main__":
    ...
    network = SGFNModel(
        config, "SceneGraphFusionNetwork", num_obj_cls, num_rel_cls
    )
    ...
    if not use_dataset:
        ...
        for i in range(n_pts):
            if counter >= edge_indices.shape[0]:
                break
        ...
        for e in range(edge_indices.shape[0]):
            ...
            for c in range(num_rel_cls):
                if adj_rel_gt[i, j, c] < 0.5:
                    continue
                rel_gt[e, c] = 1

        network.process(
            obj_points, edge_indices.t().contiguous(), obj_gt, rel_gt
        )
    ...
    for i in range(100):
        ...
        logs += network.calculate_metrics(
            [obj_pred, rel_pred], [obj_gt, rel_gt]
        )
