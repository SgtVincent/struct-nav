...
def MLP(channels: list, do_bn=True, on_last=False):
    ...
    for i in range(1, n):
        layers.append(
            torch.nn.Conv1d(
                channels[i - 1], channels[i], kernel_size=1, bias=True
            )
        )
        if i < (n - offset):
            if do_bn:
...
def build_mlp(
    dim_list, activation="relu", do_bn=False, dropout=0, on_last=False
):
    layers = []
...
class TripletGCN(MessagePassing):
    def __init__(
        self, dim_node, dim_edge, dim_hidden, aggr="add", use_bn=True
    ):
        ...
        self.nn1 = build_mlp(
            [dim_node * 2 + dim_edge, dim_hidden, dim_hidden * 2 + dim_edge],
            do_bn=use_bn,
            on_last=True,
        )
    ...
    def aggregate(
        ...
        x[0] = scatter(
            x[0], index, dim=self.node_dim, dim_size=dim_size, reduce=self.aggr
        )
        return x
...
if __name__ == "__main__":
    ...
    net = TripletGCNModel(
        num_layers, dim_node=dim_node, dim_edge=dim_edge, dim_hidden=dim_hidden
    )
