...
class GraphTripleConv(nn.Module):
    ...
    def __init__(
        ...
        assert pooling in ["sum", "avg", "wAvg"], (
            'Invalid pooling "%s"' % pooling
        )

        self.pooling = pooling
        net1_layers = [
            2 * input_dim_obj + input_dim_pred,
            hidden_dim,
    ...
    def forward(self, obj_vecs, pred_vecs, edges):
        ...
        if self.pooling == "wAvg":
            ...
            s_weights, o_weights = self.weightNet(
                new_s_vecs.detach(), new_p_vecs.detach(), new_o_vecs.detach()
            )
        ...
        if self.pooling == "wAvg":
            pooled_weight_sums = torch.zeros(
                num_objs, 1, dtype=dtype, device=device
            )
            # print(pooled_weight_sums.shape, o_idx_exp.shape, o_weights.shape)
            pooled_weight_sums = pooled_weight_sums.scatter_add(
                0, o_idx.view(-1, 1), o_weights
            )
            pooled_weight_sums = pooled_weight_sums.scatter_add(
                0, s_idx.view(-1, 1), s_weights
            )
