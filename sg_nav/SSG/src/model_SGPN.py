...
class SGPNModel(BaseModel):
    ...
    def process(
        ...
        obj_pred, rel_pred, _, _, _, _, probs = self(
            obj_points, rel_points, edges, return_meta_data=True
        )
        ...
        if self.mconfig.multi_rel_outputs:
            ...
            loss_rel = F.binary_cross_entropy(
                rel_pred, gt_rel_cls, weight=weight_FG_BG
            )
        else:
    ...
    def norm_tensor(self, points, dim):
        ...
        furthest_distance = (
            points.pow(2).sum(1).sqrt().max(1)[0]
...
if __name__ == "__main__":
    ...
    for i in range(100):
        ...
        logs, obj_pred, rel_pred = network.process(
            obj_points, rel_points, edges, obj_gt, rel_gt
        )
