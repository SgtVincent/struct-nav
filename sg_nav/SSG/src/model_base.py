...
class BaseModel(nn.Module):
    ...
    def saveConfig(self, path):
        torch.save(
            {"iteration": self.iteration, "eva_iou": self.eva_iou}, path
        )
    ...
    def save(self):
        ...
        if not os.path.exists(self.config_path + self.best_suffix):
            ...
            print(
                "current v.s. previous: {:1.3f} {:1.3f}".format(
                    self.eva_iou, eva_iou
                )
            )
        ...
        for name, model in self._modules.items():
            ...
            if skip is False:
                self.saveWeights(
                    model, os.path.join(self.saving_pth, name + suffix)
                )
        torch.save(
            {"optimizer": self.optimizer.state_dict()},
            os.path.join(self.saving_pth, "optimizer" + suffix),
        )
    ...
    def load(self, best=False):
        ...
        if os.path.exists(self.config_path + self.best_suffix) and best:
            ...
            self.config_path + self.suffix
        ) and os.path.exists(self.config_path + self.best_suffix):
            print(
                "\tNo checkpoints, but has saved best model. Load the best model"
            )
            suffix = self.best_suffix
        elif os.path.exists(self.config_path + self.suffix) and os.path.exists(
            self.config_path + self.best_suffix
        ):
            print(
                "\tFound checkpoint model and the best model. Comparing itertaion"
            )
            iteration, _ = self.loadConfig(self.config_path + self.suffix)
            iteration_best, _ = self.loadConfig(
                self.config_path + self.best_suffix
            )
            if iteration > iteration_best:
                print(
                    "\tcheckpoint has larger iteration value. Load checkpoint"
                )
                suffix = self.suffix
            else:
                print(
                    "\tthe best model has larger iteration value. Load the best model"
                )
                suffix = self.best_suffix
        ...
        self.iteration, self.eva_iou = self.loadConfig(
            self.config_path + suffix
        )
        for name, model in self._modules.items():
            skip = False
            for k in self.skip_names:
