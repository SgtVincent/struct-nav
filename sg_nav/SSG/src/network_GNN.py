...
import torch
from SSG.src.network_util import MLP, Aggre_Index, Gen_Index, build_mlp
...
class TripletEdgeNet(torch.nn.Module):
    def __init__(self, dim_node, dim_edge, use_bn=False):
        ...
        self.nn = build_mlp(
            [dim_node * 2 + dim_edge, 2 * (dim_node + dim_edge), dim_edge],
            do_bn=use_bn,
            on_last=False,
        )
...
class MultiHeadedEdgeAttention(torch.nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        self.nn_edge = build_mlp(
            [dim_node * 2 + dim_edge, (dim_node + dim_edge), dim_edge],
            do_bn=use_bn,
            on_last=False,
        )
        ...
        if self.attention == "fat":
            if use_edge:
                self.nn = MLP(
                    [d_n + d_e, d_n + d_e, d_o],
                    do_bn=use_bn,
                    drop_out=DROP_OUT_ATTEN,
                )
            else:
                self.nn = MLP(
                    [d_n, d_n * 2, d_o], do_bn=use_bn, drop_out=DROP_OUT_ATTEN
                )
    ...
    def forward(self, query, edge, value):
        ...
        edge_feature = self.nn_edge(
            torch.cat([query, edge, value], dim=1)
        )  # .view(b, -1, 1)

        if self.attention == "fat":
            value = self.proj_value(value)
            query = self.proj_query(query).view(
                batch_dim, self.d_n, self.num_heads
            )
            edge = self.proj_edge(edge).view(
                batch_dim, self.d_e, self.num_heads
            )
            if self.use_edge:
    ...
    def __init__(
        ...
        **kwargs
        ...
        if self.attention == "fat":
            ...
            self.prop = build_mlp(
                [dim_node + dim_atten, dim_node + dim_atten, dim_node],
                do_bn=use_bn,
                on_last=False,
            )
        else:
...
if __name__ == "__main__":
    ...
    if TEST_FORWARD:
        ...
        num_layers = 2
        model = GraphEdgeAttenNetworkLayers(
            dim_node,
            dim_edge,
            dim_edge,
            num_layers,
    ...
    if TEST_TRACE:
        ...
        MultiHeadedEdgeAttention(
            num_heads, dim_node, dim_edge, dim_atten
        ).trace()
