...
def build_dataset(
    config, split_type, shuffle_objs, multi_rel_outputs, use_rgb, use_normal
):
    if (
        split_type != "train_scans"
        and split_type != "validation_scans"
        and split_type != "test_scans"
    ):
    ...
    if config.dataset.dataset_type == "SGPN":
        ...
        if config.VERBOSE:
            print("build point_graph dataset")
        dataset = SGFNDataset(
            config,
            split=split_type,
            multi_rel_outputs=multi_rel_outputs,
