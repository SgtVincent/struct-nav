...
from SSG.utils import define, util, util_data, util_ply
...
def load_mesh(path, label_file, use_rgb, use_normal):
    ...
    if (
        ...
        or label_file == "labels.instances.align.annotated.ply"
    ):
        # if True:
        if use_rgb:
            plydata = util_ply.load_rgb(path, label_file)
        else:
            plydata = trimesh.load(
                os.path.join(path, label_file), process=False
            )
...
def read_relationship_json(
    data, selected_scans: list, classNames: dict, isV2: bool = True
):
    rel = dict()
    objs = dict()
    scans = list()
    ...
    for scan in data["scans"]:
        ...
        for instance_id in instances_id:
            ...
            if (
                instance_labelName in classNames
...
def dataset_loading_3RScan(
    root: str, pth_selection: str, split: str, class_choice: list = None
):
    ...
    if split == "train_scans":
        ...
        selected_scans = selected_scans.union(
            util.read_txt_to_list(
                os.path.join(pth_selection, "test_scans.txt")
            )
        )
    else:
    ...
    with open(
        os.path.join(root, "relationships_train.json"), "r"
    ) as read_file:
        data1 = json.load(read_file)
    with open(
        os.path.join(root, "relationships_validation.json"), "r"
    ) as read_file:
...
class SGFNDataset(data.Dataset):
    def __init__(
        ...
        if isinstance(self.root, list):
            ...
            (
                ...
                data,
                selected_scans,
            ) = dataset_loading_3RScan(
                self.root, self.mconfig.selection, split
            )
        ...
        (
            ...
            o_obj_cls,
            o_rel_cls,
        ) = compute_weight_occurrences.compute(
            self.classNames, self.relationNames, data, selected_scans
        )
        self.w_cls_obj = (
            torch.from_numpy(np.array(o_obj_cls))
            .float()
            .to(self.config.DEVICE)
        )
        self.w_cls_rel = (
            torch.from_numpy(np.array(o_rel_cls))
            .float()
            .to(self.config.DEVICE)
        )
        ...
        if self.config.VERBOSE:
            ...
            for i in range(len(self.classNames)):
                print(
                    "|{0:>3d} {1:>20s}".format(i, self.classNames[i]), end=""
                )
            ...
            for i in range(len(self.relationNames)):
                print(
                    "|{0:>3d} {1:>20s}".format(i, self.relationNames[i]),
                    end=" ",
                )
        ...
        (
            ...
            self.scans,
            self.nns,
        ) = self.read_relationship_json(data, selected_scans)
        if self.config.VERBOSE:
    ...
    def data_augmentation(self, points):
        # random rotate
        matrix = np.eye(3)
        matrix[0:3, 0:3] = op_utils.rotation_matrix(
        ...
        if self.use_normal:
            ...
            if self.use_rgb:
                ofset += 3
            points[:, ofset : 3 + ofset] = np.dot(
                points[:, ofset : 3 + ofset], matrix.T
            )
    ...
    def __getitem__(self, index):
        ...
        if self.sample_in_runtime:
            ...
            if self.mconfig.drop_edge > 0 and not self.for_eval:
                percentage = np.random.uniform(
                    low=1 - self.mconfig.drop_edge, high=1.0, size=1
                )[0]
                num_edge = int(float(len(edge_indices)) * percentage // 1)
                if num_edge > 0:
                    choices = np.random.choice(
                        range(len(edge_indices)), num_edge, replace=False
                    ).tolist()
            ...
            if self.max_edges > 0 and len(edge_indices) > self.max_edges:
                choices = np.random.choice(
                    range(len(edge_indices)), self.max_edges, replace=False
                ).tolist()
                edge_indices = [edge_indices[t] for t in choices]
        ...
        if self.sample_in_runtime:
            ...
            if max_edges > 0 and len(edge_indices) > max_edges:
                ...
                indices = list(
                    np.random.choice(
                        len(edge_indices), self.max_edges, replace=False
                    )
                )
                edge_indices = edge_indices[indices]

        """ random sample points """
        use_obj_context = False
        ...
        if self.multi_rel_outputs:
            adj_matrix_onehot = np.zeros(
                [len(cat), len(cat), len(self.relationNames)]
            )
        else:
        ...
        if not self.sample_in_runtime:
            ...
            if (
                max_edges > 0
                and len(edge_indices) > max_edges
                and not self.for_eval
            ):
                # for eval, do not drop out any edges.
                indices = list(
                    np.random.choice(
                        len(edge_indices), max_edges, replace=False
                    )
                )
                edge_indices = edge_indices[indices]
        ...
        if self.multi_rel_outputs:
            ...
            rel_dtype = np.int64
            adj_matrix = torch.from_numpy(
                np.array(adj_matrix, dtype=rel_dtype)
            )

        if self.multi_rel_outputs:
            gt_rels = torch.zeros(
                len(edge_indices), len(self.relationNames), dtype=torch.float
            )
        else:
    ...
    def read_relationship_json(self, data, selected_scans: list):
        ...
        for scan in data["scans"]:
            ...
            for instance_id in instances_id:
                instance_labelName = objects[instance_id]
                if (
                    instance_labelName in self.classNames
                ):  # is it a class we care about?
                    valid_counter += 1
                    # break
            if valid_counter < 2:  # need at least two nodes
...
if __name__ == "__main__":
    ...
    split_type = (
        "validation_scans"  # ['train_scans', 'validation_scans','test_scans']
    )
    dataset = SGFNDataset(
        config,
        use_rgb=True,
