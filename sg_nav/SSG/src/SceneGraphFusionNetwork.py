...
from SSG.utils import plot_confusion_matrix, util_eva
...
class SGFN:
    def __init__(self, config):
        ...
        if config.MODE == "train":
            ...
            self.dataset_valid = build_dataset(
                self.config,
        ...
        # DEBUG
            ...
            self.dataset_eval = build_dataset(
                self.config,
                split_type="validation_scans",
                shuffle_objs=False,
                multi_rel_outputs=mconfig.multi_rel_outputs,
        ...
        if self.use_edge_descriptor:
            self.model = SGFNModel(
                config, self.model_name, num_obj_class, num_rel_class
            ).to(config.DEVICE)
        else:
            # raise NotImplementedError('not yet cleaned.')
            self.model = SGPNModel(
                config, self.model_name, num_obj_class, num_rel_class
            ).to(config.DEVICE)

        self.samples_path = os.path.join(
            config.PATH, self.model_name, "samples"
        )
        self.results_path = os.path.join(
            config.PATH, self.model_name, "results"
        )
            ...
            if len(all_logs) == 1:
                if self.use_edge_descriptor:
                    ...
                    (
                        ...
                        gt_rels,
                        descriptor,
                    ) = self.data_processing(data[2:])
                    self.writter.add_graph(
                        self.model,
                        [
                            obj_points,
    ...
    def train(self):
        ...
        max_iteration = int(
            float(self.config.MAX_EPOCHES) * len(self.dataset_train)
        )
        ...
        progbar = op_utils.Progbar(
            total, width=20, stateful_metrics=["Misc/epo", "Misc/it"]
        )
        ...
        if self.model.iteration > 0:
            ...
            floored = math.floor(
                self.model.iteration / len(self.dataset_train)
            )
        ...
        eva_tool = util_eva.EvalSceneGraph(
            self.dataset_train.classNames,
            self.dataset_train.relationNames,
            multi_rel_outputs=0.5,
            k=0,
            multi_rel_prediction=self.model.mconfig.multi_rel_outputs,
        )
        eva_tool_prob = [
            util_eva.EvaPairWeight(self.dataset_valid.classNames)
        ...
        while keep_training:
            ...
            for items in loader:
                ...
                if "scene" in scan_id:
                    if "ignore_scannet_rel" in self.config.dataset:
                        ignore_none_rel = (
                            self.config.dataset.ignore_scannet_rel
                        )
                    else:
                ...
                if probs is not None:
                    if self.model.mconfig.GCN_TYPE == "EGCN":
                        for l in range(self.model.mconfig.N_LAYERS):
                            eva_tool_prob[l].update(
                                probs[l].mean(dim=1), edge_indices, gt_obj_cls
                            )

                """ calculate metrics """
                cls_, rel_ = eva_tool.get_mean_metrics()
                logs += [
                    ("IoU/train_obj_cls", cls_[0]),
                ...
                progbar.add(
                    ...
                    else [x for x in logs if not x[0].startswith("Loss")],
                )
                ...
                if (
                    self.config.LOG_IMG_INTERVAL
                    and iteration % self.config.LOG_IMG_INTERVAL == 0
                ):
                    img_confusion_matrix = (
                        plot_confusion_matrix.plot_confusion_matrix(
                            eva_tool.eva_o_cls.c_mat,
                    ...
                    self.writter.add_figure(
                        "train_obj_confusion_matrix",
                        img_confusion_matrix,
            ...
            if (
                "VALID_INTERVAL" in self.config
                and self.config.VALID_INTERVAL > 0
                and epoch % self.config.VALID_INTERVAL == 0
            ):
    ...
    def validation(self, debug_mode=False):
        ...
        progbar = op_utils.Progbar(
            total, width=20, stateful_metrics=["Misc/it"]
        )
        ...
        for i, items in enumerate(val_loader, 0):
            ...
            """ calculate metrics """
            logs = self.model.calculate_metrics(
                [pred_obj_cls, pred_rel_cls], [gt_obj_cls, gt_rel_cls]
            )
            ...
            if debug_mode:
                if i > 0:
                    break
            # break
        img_confusion_matrix = plot_confusion_matrix.plot_confusion_matrix(
            eva_tool.eva_o_cls.c_mat,
            eva_tool.eva_o_cls.class_names,
            title="Object Confusion matrix",
            plot_text=False,
    ...
    def eval(self, debug_mode=False):
        ...
        eva_tool = util_eva.EvalSceneGraph(
            ...
            self.dataset_eval.relationNames,
            multi_rel_outputs=0.5,
            k=100,
            multi_rel_prediction=self.model.mconfig.multi_rel_outputs,
        )
        ...
        for i, items in enumerate(val_loader, 0):
            ...
            [
                list_feature_maps["node_names"].append(
                    self.dataset_eval.classNames[aa]
                )
                for aa in gt_obj_cls.tolist()
            ]
            list_feature_maps["node_feature"].append(
                obj_feature.detach().cpu()
            )
            list_feature_maps["edge_feature"].append(
                rel_feature.detach().cpu()
            )
            if gcn_obj_feature is not None:
                list_feature_maps["gcn_node_feature"].append(
                    gcn_obj_feature.detach().cpu()
                )

            if not ignore_rel:
                if gcn_rel_feature is not None:
                    list_feature_maps["gcn_edge_feature"].append(
                        gcn_rel_feature.detach().cpu()
                    )
                if self.model.mconfig.multi_rel_outputs:
                    for a in range(gt_rel_cls.shape[0]):
                        ...
                        for aa in range(gt_rel_cls.shape[1]):
                            if gt_rel_cls[a][aa] > 0:
                                name += (
                                    self.dataset_eval.relationNames[aa] + "_"
                                )
                        if name == "":
                            name = "none"
                    ...
                    for a in range(gt_rel_cls.shape[0]):
                        list_feature_maps["edge_names"].append(
                            self.dataset_eval.relationNames[gt_rel_cls[a]]
                        )
        ...
        for key, item in self.model.mconfig.items():
            if (
                ...
                or isinstance(item, torch.Tensor)
            ):
                tmp_dict[key] = item
        ...
        for name, list_tensor in list_feature_maps.items():
            ...
            if not isinstance(list_tensor, list):
                continue
            if len(list_tensor) == 0:
                continue
            if not isinstance(list_tensor[0], torch.Tensor):
                continue
            ...
            if name.find("node") >= 0:
                ...
                continue
            print(name)
            self.writter.add_embedding(
                tmp,
                metadata=names,
                tag=self.model_name + "_" + name,
                global_step=self.model.iteration,
            )
    ...
    def trace(self):
        ...
        """ save relation file"""
        with open(
            os.path.join(self.trace_path, "relationships.txt"), "w"
        ) as f:
