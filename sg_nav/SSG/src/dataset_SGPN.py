...
class RIODatasetGraph(data.Dataset):
    def __init__(
        ...
        (
            ...
            self.classNames, self.relationNames, data, selected_scans, False
        )
        self.w_cls_obj = (
        ...
        self.w_cls_obj = torch.abs(
            1.0 / (torch.log(self.w_cls_obj) + 1)
        )  # +1 to prevent 1 /log(1) = inf
        self.w_cls_rel = torch.abs(
            1.0 / (torch.log(self.w_cls_rel) + 1)
        )  # +1 to prevent 1 /log(1) = inf
        ...
        for i in range(len(self.relationNames)):
            print(
                "|{0:>2d} {1:>20s}".format(i, self.relationNames[i]), end=" "
            )
