...
class SceneGraphTrainer(object):
    ...
    def __init__(self, model, num_epochs=1000, batch_size=16, val_freq=2, optimizer=None,
                scheduler=None, summary_writer=None, loss_w=1., num_objnav_class=None, device='cuda:0'):
        self.model = model
    ...
    def train(self, train_sampler, train_node_features, map_ids2idx,
             val_sampler=None, val_node_features=None, val_map_ids2idx=None,
             train_edges=None, val_edges=None):
        for i in trange(self.num_epochs, desc="Epochs: "):
            ...
            tqdm.write('Train After epoch {0} Loss:{1:0.3f}, Train Accuracy: {2:0.3f} '.format(i+1, loss_avg, acc_avg))
            if i % self.val_freq == 0 and val_sampler:
                val_loss_avg, val_acc_avg, val_iou = self.val(val_sampler, val_node_features, val_map_ids2idx, val_edges, i+1)
    ...
    def val(self, sampler, node_features, map_ids2idx, edges=None, epoch=None):
        ...
        tqdm.write('Eval After epoch {0} Loss:{1:0.3f}, Val Accuracy: {2:0.3f} '.format(epoch, loss_avg, acc_avg))
