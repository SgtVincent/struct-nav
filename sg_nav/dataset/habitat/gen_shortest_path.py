...
def gen_shortest_path_for_scene(scene_graph, save_dir):
    ...
    free_space_points = grid_xz_to_points(scene_graph.scene_bounds, idx_arr[:, [1, 0]],
                                          scene_graph.height, scene_graph.meters_per_grid)
    ...
    for object_id in tqdm(scene_graph.object_layer.obj_ids, desc="Objects:"):
        ...
        obj_geomap_and_shpath_dict = {
            "object_id": object_id,
            "object_label": scene_graph.object_layer.get_labels([object_id])[0],
            "end": end,
            "end_exact": end_exact,
            "geodesic_dist_map": np.full_like(free_space_grid, np.inf, dtype=float),
            "shortest_path_list": []
        }
...
def parse_args():
    ...
    parser.add_argument("--rel_dist_thresh", type=float, default=2.0,
...
if __name__ == "__main__":
    ...
    for scene in scene_names:
        try:
            ...
            scene_ply_path = os.path.join(args.scan_dir, scene, f'{scene}_semantic.ply')
            ...
            pcl_normals_path = os.path.join(args.scan_dir, scene, f'{scene}_normals.npy')
            house_file_path = os.path.join(args.scan_dir, scene, f'{scene}.house')
            shortest_path_dir = os.path.join(args.scan_dir, scene, f'shortest_paths')
            ...
            sg.load_gt_scene_graph(scene_ply_path, pclseg_path, pcl_normals_path, sim) 

            gen_shortest_path_for_scene(sg, save_dir=shortest_path_dir)
            
            sim.close()
            elapse = time.time() - time_start
            print(f"Generate shortest path for all free space in scene {scene}, elapse time:{elapse}")
        except: 
