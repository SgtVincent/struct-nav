...
from habitat_sim import Simulator, PathFinder
from habitat.utils.visualizations import maps

obj_nav_class_dict = {
    3: 'chair', 5: 'table', 6: 'picture', 7: 'cabinet', 8: 'cushion',
    10: 'sofa', 11: 'bed', 13: 'chest_of_drawers', 14: 'plant',
    15: 'sink', 18: 'toilet', 19: 'stool', 20: 'towel', 22: 'tv_monitor',
    23: 'shower', 25: 'bathtub', 26: 'counter', 27: 'fireplace', 
    33: 'gym_equipment', 34: 'seating', 38: 'clothes'
}

mp3d_obj_nav_class_list =[
    3, 5, 6, 7, 8, 10, 11, 13, 14, 15, 18, 19, 20, 22,
    23, 25, 26, 27, 33, 34, 38
] 

hm3d_obj_nav_class_list = [
    3, 10, 11, 14, 18, 22
]
...
def getOBB(object_dict):
    center = np.array([float(object_dict['px']), float(object_dict['py']), float(object_dict['pz'])])
    axis0 = np.array([float(object_dict['a0x']), float(object_dict['a0y']), float(object_dict['a0z'])])
    axis1 = np.array([float(object_dict['a1x']), float(object_dict['a1y']), float(object_dict['a1z'])])
    radii = np.array([float(object_dict['r0']), float(object_dict['r1']), float(object_dict['r2'])])
...
if __name__ == "__main__":
    ...
    scans_dir = "/media/junting/SSD_data/habitat_data/scene_datasets/mp3d/v1/scans"
    floor_heights_path = "/home/junting/project_cvl/SceneGraphNav/metadata/floor_heights.json"
    GENERATE_FLOOR_HEIGHT_META_FILE = False
    
    # generate floor heights meta file 
    if GENERATE_FLOOR_HEIGHT_META_FILE:
        floor_heights = get_floor_heights(scans_dir)
        with open(floor_heights_path, "w") as f:
            json.dump(floor_heights, f, indent=4)