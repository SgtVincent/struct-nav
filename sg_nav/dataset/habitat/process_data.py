...
def generate_vertex_annot(
    scene_ply, scene_glb, seg_file=None, normals_file=None
):
    ...
    object_ids = [
        int(obj.id.split("_")[-1]) for obj in sim.semantic_scene.objects
    ]
    ...
    for obj_id in object_ids:
        pcl_seg_dict[obj_id] = np.unique(
            np.concatenate(pcl_seg_dict[obj_id])
        ).astype(int)
...
if __name__ == "__main__":
    ...
    for scan in tqdm(scene_names):
        ...
        generate_vertex_annot(
            scan_ply_path,
            scan_glb_path,
            seg_file=pclseg_path,
            normals_file=normals_path,
        )
