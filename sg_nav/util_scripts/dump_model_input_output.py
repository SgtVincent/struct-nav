...
def parse_args():
    ...
    )  # test scene : 17DRP5sb8fy
    parser.add_argument("--dataset", type=str, default="matterport")
    parser.add_argument(
        "--dump_path", type=str, default="./mp3d_model_dump.pkl"
    )
    parser.add_argument(
        "--rel_dist_thresh",
        type=float,
    ...
    parser.add_argument(
        "--num_process", type=int, default=2
    )  # 10GB GPU memory
...
def process_run(feature_extractor, scan_dir, scene_name):
    ...
    scene_ply_path = os.path.join(
        scan_dir, scene_name, f"{scene_name}_semantic.ply"
    )
    scene_glb_path = os.path.join(scan_dir, scene_name, f"{scene_name}.glb")
    pclseg_path = os.path.join(
        scan_dir, scene_name, f"{scene_name}_pclseg.txt"
    )
    pcl_normals_path = os.path.join(
        scan_dir, scene_name, f"{scene_name}_normals.npy"
    )
...
if __name__ == "__main__":
    ...
    ############### serial running ###########################
    feature_extractor = SceneGraphPredictor(
        args.rel_dist_thresh, multiprocess=False
    )
    start_tick = time.time()
    ...
    stop_tick = time.time()
    print(
        f"Total time: {stop_tick - start_tick} seconds, each scene costs {(stop_tick - start_tick)/len(scene_names)} seconds"
    )
