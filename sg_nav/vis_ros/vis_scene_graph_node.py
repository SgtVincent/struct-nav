...
class VisSceneGraphNode:
    ...
    def __init__(self, node_name='vis_scene_graph_node'):
        ...
        self.scan_dir = "/media/junting/SSD_data/habitat_data/scene_datasets/mp3d/v1/scans"
        ...
        self.scene_ply_path = os.path.join(self.scan_dir, self.scene_name, f'{self.scene_name}_semantic.ply')
        self.scene_glb_path = os.path.join(self.scan_dir, self.scene_name, f'{self.scene_name}.glb')
        self.pclseg_path = os.path.join(self.scan_dir, self.scene_name, f'{self.scene_name}_pclseg.txt')
        self.pcl_normals_path = os.path.join(self.scan_dir, self.scene_name, f'{self.scene_name}_normals.npy')
    ...
    def load_data(self):
        ...
        self.scene_graph = SceneGraphHabitat(config, scene_name=self.scene_name)
        self.scene_graph.load_gt_scene_graph(self.scene_ply_path, self.pclseg_path, self.pcl_normals_path, sim)

        ############ extract GCN features by pretrained 3DSSG model 
        feature_extractor = SceneGraphPredictor(self.rel_dist_thresh)
        object_nodes = [self.scene_graph.object_layer.obj_dict[obj_id]
    ...
    def create_pcl2_msg(self, pcls, colors, a=255):
        ...
        if not np.issubdtype(type(colors[0]), int): # convert to integer if colors in [0,1]
    ...
    def publish_objects(self, vis_mode="inplace", flag_vis_name=True):
        ...
        for id, node in self.scene_graph.object_layer.obj_dict.items():
            ...
            if vis_mode == "elavated":
                center = center + np.array([0.0,0.0,self.object_elavate_node])
            ...
            if flag_vis_name:
                text_pos = center + np.array([0.0,0.0,self.object_elavate_text])
                name_marker = Marker(
    ...
    def publish_relations(self, vis_mode="inplace", color_map='viridis', vis_name=True):
        ...
        centers = [self.r_h2m @ object_node.center for object_node in object_nodes]
        ...
        for i, edge in enumerate(edges):
            ...
            if vis_mode == "elavated":
                start_center = centers[edge[0]]+np.array((0.0,0.0,self.object_elavate_node))
                end_center = centers[edge[1]]+np.array((0.0,0.0,self.object_elavate_node))
            else: # "inplace":
            ...
            if vis_name:
                text_pos=(start_center + end_center)/2.0 + \
                    np.array((0,0,self.object_elavate_text))
                name_marker = Marker( 
                    type=Marker.TEXT_VIEW_FACING,
                    id=i,
                    ns=self.relation_names[rel],
