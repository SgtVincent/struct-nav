...
def calculate_mesh(mesh_msg: Mesh):
    ...
    for mesh_block in mesh_msg.mesh_blocks:
        ...
        mesh_x = (
            np.array(mesh_block.x, dtype=float) * point_conv_factor
            + float(index[0])
        ) * block_edge_length
        mesh_y = (
            np.array(mesh_block.y, dtype=float) * point_conv_factor
            + float(index[1])
        ) * block_edge_length
        mesh_z = (
            np.array(mesh_block.z, dtype=float) * point_conv_factor
            + float(index[2])
        ) * block_edge_length
