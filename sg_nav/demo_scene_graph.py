...
def parse_args():
    ...
    parser.add_argument(
        "--scene_name", type=str, default="fzynW3qQPVF"
    )  # fzynW3qQPVF 17DRP5sb8fy
    parser.add_argument("--dataset", type=str, default="matterport")
    parser.add_argument("--visualize", type=bool, default=True)
    # runtime options
    parser.add_argument(
        "--mode", type=str, default="runtime", choices=["runtime", "offline"]
    )
    parser.add_argument(
        "--rel_dist_thresh",
        type=float,
    ...
    # offline options
    parser.add_argument(
        "--scene_graph_dump_path",
        type=str,
        default="/home/junting/project_cvl/SceneGraphNav/data/model_dump/mp3d_model_dump.pkl",
    )

    args = parser.parse_args()
    args.scene_ply_path = os.path.join(
        args.scan_dir, args.scene_name, f"{args.scene_name}_semantic.ply"
    )
    args.scene_glb_path = os.path.join(
        args.scan_dir, args.scene_name, f"{args.scene_name}.glb"
    )
    args.pclseg_path = os.path.join(
        args.scan_dir, args.scene_name, f"{args.scene_name}_pclseg.txt"
    )
    args.pcl_normals_path = os.path.join(
        args.scan_dir, args.scene_name, f"{args.scene_name}_normals.npy"
    )
...
if __name__ == "__main__":
    ...
    if args.mode == "runtime":
        ...
        scene_graph.load_gt_scene_graph(
            args.scene_ply_path, args.pclseg_path, args.pcl_normals_path, sim
        )
