# Credit to https://github.com/facebookresearch/votenet/blob/main/models/ap_helper.py
...
def parse_predictions(end_points, config_dict):
    ...
    pred_sem_cls = torch.argmax(
        end_points["sem_cls_scores"], -1
    )  # B,num_proposal
...
def parse_groundtruths(end_points, config_dict):
    ...
    for i in range(bsize):
        batch_gt_map_cls.append(
            [
                (
                    sem_cls_label[i, j].item(),
                    gt_corners_3d_upright_camera[i, j],
                )
...
class APCalculator(object):
    ...
    def compute_metrics(self):
        ...
        for key in sorted(ap.keys()):
            clsname = (
                self.class2type_map[key] if self.class2type_map else str(key)
            )
