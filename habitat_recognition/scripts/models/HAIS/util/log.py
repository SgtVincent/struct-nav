...
def create_logger(log_file):
    ...
    logging.basicConfig(
        level=logging.DEBUG, format=log_format, filename=log_file
    )  # filename: build a FileHandler
    return logger
...
if cfg.task == "train":
    ...
    log_file = os.path.join(
        cfg.exp_path,
        "result",
        "epoch{}_nmst{}_scoret{}_npointt{}".format(
            cfg.test_epoch,
            cfg.TEST_NMS_THRESH,
        ...
        cfg.split,
        "test-{}.log".format(time.strftime("%Y%m%d_%H%M%S", time.localtime())),
    )
