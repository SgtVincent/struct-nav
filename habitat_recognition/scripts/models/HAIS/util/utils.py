...
def cosine_lr_after_step(
    optimizer, base_lr, epoch, step_epoch, total_epochs, clip=1e-6
):
    if epoch < step_epoch:
        lr = base_lr
    else:
        ...
        if "net" in checkpoint.keys() and "optimizer" in checkpoint.keys():
            ...
            if optimizer is not None and load_optimizer == True:
                ...
                for k in optimizer.state.keys():
                    optimizer.state[k]["exp_avg"] = optimizer.state[k][
                        "exp_avg"
                    ].cuda()
                    optimizer.state[k]["exp_avg_sq"] = optimizer.state[k][
                        "exp_avg_sq"
                    ].cuda()
            ...
            for k, v in checkpoint.items():
                if "module." in k:
                    checkpoint = {
                        k[len("module.") :]: v for k, v in checkpoint.items()
                    }
                break
            if dist:
