...
def test(model, model_fn, data_name, epoch):
    ...
    with torch.no_grad():
        ...
        for i, batch in enumerate(dataloader):
            ...
            test_scene_name = dataset.test_file_names[
                int(batch["id"][0])
            ].split("/")[-1][:12]
            ...
            if epoch > cfg.prepare_epochs:
                ...
                proposals_idx, proposals_offset, mask_scores = preds[
                    "proposals"
                ]
                ...
                semantic_id = torch.tensor(
                    semantic_label_idx, device=scores_pred.device
                )[
                    semantic_pred[
                        proposals_idx[:, 1][
                            proposals_offset[:-1].long()
                        ].long()
                    ]
                ]  # (nProposal), long
                ...
                if getattr(cfg, "using_NMS", False):
                    if semantic_id.shape[0] == 0:
                        ...
                        proposals_pn_h = proposals_pointnum.unsqueeze(
                            -1
                        ).repeat(1, proposals_pointnum.shape[0])
                        proposals_pn_v = proposals_pointnum.unsqueeze(
                            0
                        ).repeat(proposals_pointnum.shape[0], 1)
            ...
            if cfg.save_semantic:
                os.makedirs(
                    os.path.join(result_dir, "semantic"), exist_ok=True
                )
                semantic_np = semantic_pred.cpu().numpy()
                np.save(
                    os.path.join(
                        result_dir, "semantic", test_scene_name + ".npy"
                    ),
                    semantic_np,
                )
            ...
            if cfg.save_pt_offsets:
                os.makedirs(
                    os.path.join(result_dir, "coords_offsets"), exist_ok=True
                )
                pt_offsets_np = pt_offsets.cpu().numpy()
                coords_np = batch["locs_float"].numpy()
                coords_offsets = np.concatenate(
                    (coords_np, pt_offsets_np), 1
                )  # (N, 6)
                np.save(
                    os.path.join(
                        result_dir, "coords_offsets", test_scene_name + ".npy"
                    ),
                    coords_offsets,
                )
            ...
            if epoch > cfg.prepare_epochs and cfg.save_instance:
                f = open(
                    os.path.join(result_dir, test_scene_name + ".txt"), "w"
                )
                for proposal_id in range(nclusters):
                    ...
                    f.write(
                        "predicted_masks/{}_{:03d}.txt {} {:.4f}".format(
                            test_scene_name,
                            proposal_id,
                            semantic_label_idx[semantic_label],
                            score,
                        )
                    )
            ...
            logger.info(
                "instance iter: {}/{} point_num: {} ncluster: {} inference time: {:.2f}s".format(
                    batch["id"][0] + 1,
        ...
        logger.info(
            "whole set inference time: {:.2f}s, latency per frame: {:.2f}ms".format(
                total_end1, total_end1 / len(dataloader) * 1000
            )
        )
        ...
        if cfg.split == "val":
            ...
            logger.info(
                "semantic_segmantation_accuracy: {:.4f}".format(seg_accuracy)
            )
...
def evaluate_semantic_segmantation_accuracy(matches):
    ...
    correct = (
        seg_gt_all[seg_gt_all != -100] == seg_pred_all[seg_gt_all != -100]
    ).sum()
...
def evaluate_semantic_segmantation_miou(matches):
    ...
    for _index in seg_gt_all.unique():
        if _index != -100:
            intersection = (
                (seg_gt_all == _index) & (seg_pred_all == _index)
            ).sum()
