...
input_file = os.path.join(
    arg.data_path, arg.data_split, arg.room_name + "_inst_nostuff.pth"
)
...
instance_file = os.path.join(
    arg.prediction_path, arg.data_split, arg.room_name + ".txt"
)
assert os.path.isfile(instance_file), "No instance result - {}.".format(
    instance_file
)
...
for i in range(len(masks)):
    ...
    print(
        "{} {}: {} pointnum: {}".format(
            i, masks[i], SEMANTIC_IDX2NAME[int(masks[i][1])], mask.sum()
        )
    )
