...
class HAIS(nn.Module):
    def __init__(self, cfg):
        ...
        self.output_layer = spconv.SparseSequential(norm_fn(width), nn.ReLU())
        ...
        self.intra_ins_unet = UBlock(
            [width, 2 * width], norm_fn, 2, block, indice_key_id=11
        )
        self.intra_ins_outputlayer = spconv.SparseSequential(
            norm_fn(width), nn.ReLU()
        )
        ...
        module_map = {
            ...
            "intra_ins_unet": self.intra_ins_unet,
            "intra_ins_outputlayer": self.intra_ins_outputlayer,
            "score_linear": self.score_linear,
            "mask_linear": self.mask_linear,
        }
    ...
    def clusters_voxelization(
        ...
        clusters_coords_mean = hais_ops.sec_mean(
        ...
        clusters_coords_min = hais_ops.sec_min(
            clusters_coords, clusters_offset.cuda()
        )  # (nCluster, 3), float
        clusters_coords_max = hais_ops.sec_max(
            clusters_coords, clusters_offset.cuda()
        )  # (nCluster, 3), float

        clusters_scale = (
            1
            / ((clusters_coords_max - clusters_coords_min) / fullscale).max(1)[
                0
            ]
            - 0.01
        )  # (nCluster), float
        ...
        offset = torch.index_select(
            offset, 0, clusters_idx[:, 0].cuda().long()
        )
        ...
        out_coords, inp_map, out_map = hais_ops.voxelization_idx(
        ...
        out_feats = hais_ops.voxelization(
    ...
    def forward(
        ...
        semantic_scores = self.semantic_linear(
            output_feats
        )  # (N, nClass), float
        ...
        if epoch > self.prepare_epochs:
            ...
            if self.cfg.dataset == "scannetv2":
                object_idxs = torch.nonzero(semantic_preds > 1).view(
                    -1
                )  # floor idx 0, wall idx 1
            else:
                raise Exception

            # fliter out floor and wall
            batch_idxs_ = batch_idxs[object_idxs]
            batch_offsets_ = utils.get_batch_offsets(
                batch_idxs_, input.batch_size
            )
            ...
            idx, start_len = hais_ops.ballquery_batch_p(
                coords_ + pt_offsets_,
                batch_idxs_,
                batch_offsets_,
                self.point_aggr_radius,
                self.cluster_shift_meanActive,
            )

            using_set_aggr_in_training = getattr(
                self.cfg, "using_set_aggr_in_training", True
            )
            using_set_aggr_in_testing = getattr(
                self.cfg, "using_set_aggr_in_testing", True
            )
            using_set_aggr = (
                using_set_aggr_in_training
                if training_mode == "train"
                else using_set_aggr_in_testing
            )

            (
                proposals_idx,
                proposals_offset,
            ) = hais_ops.hierarchical_aggregation(
                semantic_preds_cpu,
            ...
            if (
                training_mode == "train"
                and proposals_offset.shape[0] > max_proposal_num
            ):
            ...
            # predict instance scores
            if (
                getattr(self.cfg, "use_mask_filter_score_feature", False)
                and epoch > self.cfg.use_mask_filter_score_feature_start_epoch
            ):
                mask_index_select = torch.ones_like(mask_scores)
                mask_index_select[
                    torch.sigmoid(mask_scores)
                    < self.cfg.mask_filter_score_feature_thre
                ] = 0.0
                score_feats = score_feats * mask_index_select
            score_feats = hais_ops.roipool(
...
def model_fn_decorator(test=False):
    ...
    def test_model_fn(batch, model, epoch):
        ...
        voxel_feats = hais_ops.voxelization(
            feats, v2p_map, cfg.mode
        )  # (M, C), float, cuda

        input_ = spconv.SparseConvTensor(
            voxel_feats, voxel_coords.int(), spatial_shape, cfg.batch_size
        )
        ...
        if epoch > cfg.prepare_epochs:
            scores, proposals_idx, proposals_offset, mask_scores = ret[
                "proposal_scores"
            ]
    ...
    def model_fn(batch, model, epoch):
        ...
        instance_labels = batch[
            "instance_labels"
        ].cuda()  # (N), long, cuda, 0~total_nInst, -100

        instance_info = batch[
            "instance_info"
        ].cuda()  # (N, 9), float32, cuda, (meanxyz, minxyz, maxxyz)
        instance_pointnum = batch[
            "instance_pointnum"
        ].cuda()  # (total_nInst), int, cuda
        batch_offsets = batch["offsets"].cuda()  # (B + 1), int, cuda
        spatial_shape = batch["spatial_shape"]
    ...
    def loss_fn(loss_inp, epoch):
        ...
        """offset loss"""
        pt_offsets, coords, instance_info, instance_labels = loss_inp[
            "pt_offsets"
        ]
        ...
        offset_norm_loss = torch.sum(pt_dist * valid) / (
            torch.sum(valid) + 1e-6
        )
        ...
        if epoch > cfg.prepare_epochs:
            ...
            if getattr(cfg, "cal_iou_based_on_mask", False) and (
                epoch > cfg.cal_iou_based_on_mask_start_epoch
            ):
                ious, mask_label = hais_ops.cal_iou_and_masklabel(
            ...
            mask_label[mask_label == -1.0] = 0.5  # any value is ok
            mask_loss = torch.nn.functional.binary_cross_entropy(
                mask_scores_sigmoid,
                mask_label,
                weight=mask_label_weight,
                reduction="none",
            )
            ...
            gt_scores = get_segmented_scores(
                gt_ious, cfg.fg_thresh, cfg.bg_thresh
            )

            score_loss = score_criterion(
                torch.sigmoid(scores.view(-1)), gt_scores
            )
        ...
        """total loss"""
        loss = (
            cfg.loss_weight[0] * semantic_loss
            + cfg.loss_weight[1] * offset_norm_loss
        )
        if epoch > cfg.prepare_epochs:
