...
def train_epoch(train_loader, model, model_fn, optimizer, epoch):
    ...
    for i, batch in enumerate(train_loader):
        ...
        utils.cosine_lr_after_step(
            optimizer, cfg.lr, epoch - 1, cfg.step_epoch, cfg.epochs
        )
    ...
    logger.info(
        "epoch: {}/{}, train loss: {:.4f}, time: {}s".format(
            epoch, cfg.epochs, am_dict["loss"].avg, time.time() - start_epoch
        )
    )
...
def eval_epoch(val_loader, model, model_fn, epoch):
    ...
    with torch.no_grad():
        ...
        logger.info(
            "epoch: {}/{}, val loss: {:.4f}, time: {}s".format(
                epoch,
                cfg.epochs,
                am_dict["loss"].avg,
...
if __name__ == "__main__":
    ...
    for epoch in range(start_epoch, cfg.epochs + 1):
        train_epoch(
            dataset.train_data_loader, model, model_fn, optimizer, epoch
        )
