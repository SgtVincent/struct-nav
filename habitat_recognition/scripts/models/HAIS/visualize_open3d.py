...
def get_coords_color(opt):
    input_file = os.path.join(
        opt.data_path, opt.data_split, opt.room_name + "_inst_nostuff.pth"
    )
    assert os.path.isfile(input_file), "File not exist - {}.".format(
        input_file
    )
    ...
    if opt.task == "semantic_gt":
        ...
        assert os.path.isfile(
            semantic_file
        ), "No semantic result - {}.".format(semantic_file)
        ...
        assert os.path.isfile(offset_file), "No offset result - {}.".format(
            offset_file
        )
    ...
    # same color order according to instance pointnum
        ...
        instance_file = os.path.join(
            opt.prediction_path, opt.data_split, opt.room_name + ".txt"
        )
        assert os.path.isfile(
            instance_file
        ), "No instance result - {}.".format(instance_file)
        ...
        for i in range(len(masks) - 1, -1, -1):
            mask_path = os.path.join(
                opt.prediction_path, opt.data_split, masks[i][0]
            )
            ...
            print(
                "{} {}: {} pointnum: {}".format(
                    i,
                    masks[i],
                    SEMANTIC_IDX2NAME[int(masks[i][1])],
                    mask.sum(),
                )
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--data_path",
        default="./dataset/scannetv2",
        help="path to the dataset files",
    )
    parser.add_argument(
        "--prediction_path",
        default="./exp/scannetv2/hais/hais_eval_scannet/result",
        help="path to the prediction results",
    )
    parser.add_argument(
        "--data_split", help="train / val / test", default="val"
    )
    parser.add_argument(
        "--room_name", help="room_name", default="scene0011_00"
    )
    parser.add_argument(
        "--task",
