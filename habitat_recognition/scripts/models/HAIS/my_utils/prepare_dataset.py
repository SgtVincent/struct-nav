...
def parse_args():
    ...
    parser.add_argument("--meta_dir", type=str, default="dataset/scannetv2/meta_data")
    parser.add_argument("--src_scans_dir", type=str, default="/media/junting/SSD_data/ScanNet/scans")
    parser.add_argument("--src_scans_test_dir", type=str, default="/media/junting/SSD_data/ScanNet/scans_test")
    parser.add_argument("--dst_dataset_dir", type=str, default="/home/junting/project_cvl/HAIS/dataset/scannetv2")
    parser.add_argument("--mode", type=str, default="soft_link", choices=["soft_link", "copy"])
...
if __name__ == "__main__":
    ...
    if args.mode == "soft_link":
        for scene in train_scenes:
            ...
            os.symlink(os.path.join(scene_dir, f"{scene}_vh_clean_2.ply"), 
                os.path.join(train_dataset_dir, f"{scene}_vh_clean_2.ply")) 
            os.symlink(os.path.join(scene_dir, f"{scene}_vh_clean_2.labels.ply"), 
                os.path.join(train_dataset_dir, f"{scene}_vh_clean_2.labels.ply")) 
            os.symlink(os.path.join(scene_dir, f"{scene}_vh_clean_2.0.010000.segs.json"), 
                os.path.join(train_dataset_dir, f"{scene}_vh_clean_2.0.010000.segs.json")) 
            os.symlink(os.path.join(scene_dir, f"{scene}.aggregation.json"),
        ...
        for scene in val_scenes:
            ...
            os.symlink(os.path.join(scene_dir, f"{scene}_vh_clean_2.labels.ply"), 
                os.path.join(val_dataset_dir, f"{scene}_vh_clean_2.labels.ply")) 
            os.symlink(os.path.join(scene_dir, f"{scene}_vh_clean_2.0.010000.segs.json"), 
                os.path.join(val_dataset_dir, f"{scene}_vh_clean_2.0.010000.segs.json")) 
            os.symlink(os.path.join(scene_dir, f"{scene}.aggregation.json"),
