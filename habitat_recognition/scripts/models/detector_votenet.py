...
from utils.votenet_utils import parse_predictions, parse_groundtruths
...
class DetectorVoteNet():
    def __init__(self, config, device="cuda") -> None:
        ...
        # initialize model
        self.config = config
        self.net = VoteNet(num_class=config.num_class,
        ...
        self.net.to(device)
        self.parse_predict_config = {'num_class':config.num_class, 'remove_empty_box': (not config.faster_eval), 
            'use_3d_nms': config.use_3d_nms, 'nms_iou': config.nms_iou,'use_old_type_nms': config.use_old_type_nms, 
            'cls_nms': config.use_cls_nms, 'per_class_proposal': config.per_class_proposal, 'conf_thresh': config.conf_thresh, 'mean_size_arr': config.mean_size_arr}

        # initialize evaluator
        self.ap_calculators = {iou_thresh: APCalculator(iou_thresh) for iou_thresh in self.config.ap_iou_thresholds}
        if self.config.eval_realtime:
            self.evaluator = Eval3DDetection(
                self.config,
                self.ap_calculators)
    ...
    def generate_input(self, o3d_pcd):
        ...
        if self.config.use_height:
            ...
            point_cloud = np.concatenate([pcd_xyz, np.expand_dims(height, 1)],1) 
        ...
        input = {}
        point_cloud = torch.from_numpy(point_cloud.astype(np.float32)).to(self.device)
        input['point_clouds'] = torch.unsqueeze(point_cloud, 0) # add batch dimension
        return input
