...
from utils.mp3d_utils import read_label_mapping
from utils.box_util import get_3d_box
import pathlib
PACKAGE_DIR = pathlib.Path(__file__).parent.parent.parent.absolute() # package dir 
...
class Eval3DDetection():
    def __init__(self,
        ...
        self.gt_map_cls = []
        self.label_mapping_mp40_nyu40 = read_label_mapping(self.label_mapping_file,label_from='mpcat40',label_to='nyu40id')
        
        for obj in self.semantic_scene.objects:
            
            # parse votenet label
            if obj.category.name() in self.label_mapping_mp40_nyu40:
                gt_nyu40_id = self.label_mapping_mp40_nyu40[obj.category.name()]
        ...
        self.gt_map_cls = [self.gt_map_cls] # aligned with shape of pred_map_cls
    ...
    def eval(self, pred_map_cls):
        ...
        for iou_thresh in self.ap_calculators.keys():
            ...
            if self.logging:
                ...
                self.logger.add_scalar(f"mean AP @{iou_thresh}", metrics_dict["mAP"], x_axis)
                self.logger.add_scalar(f"mean Recall @{iou_thresh}", metrics_dict["AR"], x_axis)
                ...
                for key, val in metrics_dict.items():
                    if "Average Precision" in key: 
                        ...
                        self.logger.add_scalar(f"Average Precision @{iou_thresh}/{class_name}", val, x_axis)
                    elif "Recall" in key:
                        class_name = rc_re.findall(key)[0]
                        self.logger.add_scalar(f"Recall @{iou_thresh}/{class_name}", val, x_axis)
                    else: # skip
                        pass
