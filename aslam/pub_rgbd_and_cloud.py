...
"""This script publishs rgbd images, camera info and point cloud."""
...
from .utils.lib_rgbd import MyCameraInfo, create_o3dpc_from_rgbd
...
def pre_base(relative_path, base_dir):
    """Pre-Append base_dir to the relative path."""
...
def parse_command_line_argumetns():
    ...
    parser.add_argument(
        ...
        "If the path is relative, this ROS package's \
        path will be inserted to the beginning. "
        "Default='config/rgbd_pub_config.yaml'",
    )
    ...
    args.config_file = pre_base(args.config_file, args.base_dir)
    return args
...
class RgbdDataLoader:
    ...
    def __init__(self, config, base_dir):
        """Arguments: config {dict}: the output from `read_config_file()`."""
        self._config = config
        self._base_dir = base_dir
        self.reset()

    def read_next_data(self):
        """Read next data from loader."""
        ...
        # Create point cloud.
        if self.is_pub_cloud():
            cloud = create_o3dpc_from_rgbd(
                color, depth, camera_info, self._depth_unit
            )
    ...
    def has_data(self):
        ...
        Return False if: Image counter >= total color images.
        Or: Image counter >= total depth images.
        """
    ...
    def reset(self):
        ...
        _is_pub_color = config["color"]["is_publish"]
        _is_pub_depth = config["depth"]["is_publish"]
        _is_pub_camera_info = config["camera_info"]["is_publish"]
        _is_pub_cloud = config["point_cloud"]["is_publish"]
        self._depth_unit = float(config["depth"]["depth_unit"])

        color_folder = (
            pre_base(config["color"]["folder"], self._base_dir) + "/"
        )
        depth_folder = (
            pre_base(config["depth"]["folder"], self._base_dir) + "/"
        )
        camera_info_file_path = pre_base(
            config["camera_info"]["file"], self._base_dir
        )

        # -- Whether read data or not.
        # If we need to publish data, then we need to read it first.
        # Or, if need point cloud, we need to read color/depth/camera_info.
        self._is_read_color = _is_pub_color or _is_pub_cloud
        self._is_read_depth = _is_pub_depth or _is_pub_cloud
        self._is_read_camera_info = _is_pub_camera_info or _is_pub_cloud
    ...
    def is_pub_color(self):
        """Publish color images or not."""
        return self._config["color"]["is_publish"]

    def is_pub_depth(self):
        """Publish depth images or not."""
        return self._config["depth"]["is_publish"]

    def is_pub_camera_info(self):
        """Publish camera info or not."""
        return self._config["camera_info"]["is_publish"]

    def is_pub_cloud(self):
        """Publish point cloud or not."""
        return self._config["point_cloud"]["is_publish"]
...
def main(config, base_dir):
    ...
    Args:
        config (dict): Read from a config file such as
        `config/run_publisher.launch`.
        base_dir (str): Base direactory.
    """
    # -- Set data loader
    rgbd_data = RgbdDataLoader(config, base_dir)
    ...
    while not rospy.is_shutdown():
        ...
        rospy.loginfo("=================================================")
        rospy.loginfo(
            "Publish {}/{}th data; {} published in total.".format(
        ...
        if rgbd_data.is_pub_color():
            ...
            rospy.loginfo(
                "Publish color image, " "shape = {}.".format(color.shape)
            )
        ...
        if rgbd_data.is_pub_depth():
            ...
            rospy.loginfo(
                "Publish depth image, " "shape = {}.".format(depth.shape)
            )
        ...
        if rgbd_data.is_pub_cloud():
            pub_cloud.publish(cloud, cloud_format="open3d", frame_id=frame_id)

            num_points = np.asarray(cloud.points).shape[0]
            rospy.loginfo(
                "Publish point cloud, " "{} points.".format(num_points)
            )
...
if __name__ == "__main__":
    ...
    my_args = parse_command_line_argumetns()
    my_config = read_config_file(my_args.config_file)
    main(my_config, my_args.base_dir)
