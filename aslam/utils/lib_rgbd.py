...
class MyCameraInfo:
    ...
    def __init__(self, camera_info_file_path=None, ros_camera_info=None):
    ...
    def resize(self, ratio):
        ...
        self._intrinsic_matrix[:-1] = [
            x * ratio for x in self._intrinsic_matrix[:-1]
        ]
    ...
    def to_open3d_format(self):
        ...
        Format: `class open3d.camera.PinholeCameraIntrinsic`.
        Reference: http://www.open3d.org/docs/release/python_api/
            open3d.camera.PinholeCameraIntrinsic.html
        """
    ...
    @staticmethod
    def _from_ros_camera_info(ros_camera_info):
        """Get camera data from ros format."""
        k_mat = ros_camera_info.K  # ROS is row majored.
        # However, here I'm using column majored.
        data = {
            "width": ros_camera_info.width,
            "height": ros_camera_info.height,
            "intrinsic_matrix": get_intrinsic_mat(k_mat),
        }
        return data


def create_o3dpc_from_rgbd(color_img, depth_img, cam_info, depth_unit=0.001):
    """Create pointcreate_open3dpoint_cloud_from_rgbd cloud of \
    open3d format, given opencv rgbd images and camera info.

    Refer to http://www.open3d.org/docs/release/python_api/
            open3d.geometry.PointCloud.html.

    Args:
        color_img ([np.ndarry, np.uint8]): 3 channels of BGR. Undistorted.
        depth_img ([np.ndarry, np.uint16]): Undistorted depth image \
            that matches color_img.
        cam_info (CameraInfo): Camera Information.
        depth_unit (float): If depth_img[i, j] is x, then the real depth is \
            x*depth_unit meters.
    :returns:
